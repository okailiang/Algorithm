...
public class DNACompose {
    ...
    private static int DNACompose(String dna1, String dna2) {
        int dnaLen = dna1.length();
        int result = 0;
        while (--dnaLen >= 0) {
            if (dna1.charAt(dnaLen) == 'A' && dna2.charAt(dnaLen) == 'T') {
                continue;
            }
            if (dna1.charAt(dnaLen) == 'T' && dna2.charAt(dnaLen) == 'A') {
                continue;
            }
            if (dna1.charAt(dnaLen) == 'C' && dna2.charAt(dnaLen) == 'G') {
                continue;
            }
            if (dna1.charAt(dnaLen) == 'G' && dna2.charAt(dnaLen) == 'C') {
                continue;
            }
            result = result + 1;
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(DNACompose(strArray[0], strArray[1]));
    }
}
